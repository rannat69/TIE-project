...
app.post("/updates/:id/comments", requireFirebaseAuth, async (req, res) => {
  ...
  const courseId = (projSnap.get("courseId") as string | undefined) ?? "";
  const instructorIds =
    ((await db.collection("courses").doc(courseId).get()).get(
      "instructorIds",
    ) as string[] | undefined) ?? [];

  const canComment =
    studentIds.includes(u.id) ||
    instructorIds.includes(u.id) ||
    u.role === "admin";
  if (!canComment)
    return res
      .status(403)
...
app.post("/cas/serviceValidate", async (req, res) => {
  const getServiceValidate = async () => {
    ...
    const parser = new XMLParser({
      removeNSPrefix: true, // Drops 'cas:' from all keys
    });
    const jsonObj = parser.parse(xmlString);
    if (jsonObj.serviceResponse.authenticationSuccess?.user) {
      return jsonObj.serviceResponse.authenticationSuccess;
    } else {
      return null;
    }
  };
  ...
  console.log("resServiceValidate", resServiceValidate);
